...
//! ```
//!
//! The source's un-pitch pass already drops alpha (pitched-RGBA → tight RGB), so
//! there's no separate preprocess copy — the frame goes straight to the detector.
//! Every buffer is allocated once and reused; the frame never leaves the GPU;
//! stages 1–2 only *enqueue* work, mirroring TensorRT's `enqueueV3` → one
//! `cudaStreamSynchronize` → read.
//!
...
//!
//! Gated behind the `rfdetr-example` feature (pulls the private vrt-rfdetr dep);
//! build needs `export CARGO_NET_GIT_FETCH_WITH_CLI=true`.
//!
//! Usage:
//!   cargo run --release -p sensor-rtsp --example rtsp_rfdetr \
//!       --features rfdetr-example -- rtsp://<camera>/stream [conf]
